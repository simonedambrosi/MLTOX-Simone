...
# unsupervised step
def data_rasar(train_distance_matrix, test_distance_matrix, X_train, X_test, y_train, y_test):
    ######## starting DATAFRAME ##########
...
def cv_simple_rasar(X, y):
    ...
    for train_index, test_index in kf.split(distance_matrix):
        ...
        rasar_train, rasar_test = data_rasar(dist_matr_train, dist_matr_test, X.iloc[train_index], X.iloc[test_index],
                                             y_train, y_test)
